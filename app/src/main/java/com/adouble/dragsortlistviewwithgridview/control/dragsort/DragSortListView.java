...
package com.adouble.dragsortlistviewwithgridview.control.dragsort;
...
public class DragSortListView extends ListView {
    ...
    public DragSortListView(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            mParentOrChild = a.getInt(R.styleable.DragSortListView_parent_or_child, mParentOrChild);

            if (useDefault) {
                isChildClick = a.getBoolean(R.styleable.DragSortListView_ischild_click, false);
                ...
                controller.setIsChildClick(isChildClick);
    ...
    public void setAdapter1(ListAdapter adapter) {
        if (adapter != null) {
            mAdapterWrapper = new AdapterWrapper(adapter);
            if (adapter instanceof DropListener) {
                setDropListener((DropListener) adapter);
    ...
    private static int buildRunList(SparseBooleanArray cip, int rangeStart,
        ...
        for (i++; i < cip.size() && (position = cip.keyAt(i)) < rangeEnd; i++) {
            ...
            if (position == currentRunEnd) {
                ...
                if (runCount > 0 && runCount < runStart.length)
                    runStart[runCount] = currentRunStart;
                if (runCount > 0 && runCount < runEnd.length)
